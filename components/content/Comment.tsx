    ...
    <div className="mx-1 p-1 border border-gray-200 rounded-lg bg-slate-100 dark:bg-slate-800 dark:border-gray-700 mb-4" data-cy={`Comment:${comment.id}:Main`}>
      ...
      <div data-cy={`Comment:${comment.id}:Editing`}>
        ...
        <Stack direction='row-reverse'>
          {!isPlaceholder && (
            <TinyButton onClick={handleSubmit(onSubmit)} disabled={!watchMarkdown}><MdSave data-cy={`Comment:${comment.id}:Save`} /></TinyButton>
          )}
          {isPlaceholder && saveComment && (
            <TinyButton onClick={handleSubmit(onSubmitPlaceholder)} disabled={!watchMarkdown}><MdSave data-cy={`Comment:${comment.id}:Save`} /></TinyButton>
          )}
        </Stack>
      </div>
      ) : (
