...
const useUsersOnEvent = (eventId: number | undefined): { users: UsersWithUserOnEvents[] | undefined, error: string, isLoading: boolean, mutate: KeyedMutator<Data> } => {
  const { data, error, isLoading, mutate} = useSWR(eventId ? `${basePath}/api/event/${eventId}/users` : false, usersFetcher)
