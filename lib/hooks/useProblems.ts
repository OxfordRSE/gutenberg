...
export const useProblems = (eventId: number | undefined): { problems: Problem[] | undefined, error: string, isLoading: boolean, mutate: KeyedMutator<Data> } => {
  const { data, isLoading, error, mutate } = useSWR(eventId ? `${basePath}/api/event/${eventId}/problems` : false, problemsFetcher)
