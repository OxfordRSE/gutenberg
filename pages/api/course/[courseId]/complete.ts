...
const completeHandler = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  const updated: PublicUserOnCourse = await prisma.userOnCourse.upsert({
    where: { userEmail_courseId: { userEmail, courseId } },
    update: { status: CourseStatus.COMPLETED, completedAt: new Date() },
    create: { userEmail, courseId, status: CourseStatus.COMPLETED, completedAt: new Date() },
